...
namespace galgo {
    ...
    template <typename T>
    T Chromosome<T>::get_value(int k) const
    {
#ifndef NDEBUG
        if (k < 0 || k >= ptr->nbparam) {
            throw std::invalid_argument("Error: in galgo::Chromosome<T>::get_value(int), first argument cannot be outside interval [0,nbparam-1], please amend.");
        }
#endif
